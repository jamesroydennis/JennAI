...
from admin.setup_env import setup_environment_file
...
def main_setup():
    ...
    # --- 1. Setup .env file ---
    logger.info("Step 1: Setting up local environment file (.env)...")
    try:
        env_setup_exit_code = setup_environment_file()
        if env_setup_exit_code != 0:
            logger.error("Local environment file setup failed. Aborting.")
            all_successful = False
    except Exception as e:
        logger.critical(f".env setup script encountered an unexpected error: {e}")
        all_successful = False

    # --- 2. Run Cleanup Script ---
    if all_successful:
        logger.info("Step 2: Running cleanup script...")
        try:
    ...
    # --- 3. Create Project Folders and __init__.py files ---
    if all_successful: # Only proceed if previous steps were successful
        logger.info("Step 3: Setting up project folder structure...")
        try:
